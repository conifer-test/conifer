...
const runTestsInParallel = async () => {
  ...
  const taskCommands = taskArns.map((taskArn, index) => {
    const taskParams = {
      ...
      // overrides: [testRunId],
      // overrides: [JSON.stringify(testRunId)],
    };
...
const areTasksRunning = async (taskArns) => {
  ...
  console.log(response.tasks.map(task => task.lastStatus).every(task => task === 'STOPPED'))
  // return response.tasks.map(task => task.lastStatus);
